...
def libcurlSharedLib := match get_config? libcurlSharedLib with | some v => v | none => "/usr/lib/libcurl.so.4"
...
target leancurl.o pkg : FilePath := do
  let dirPrefix := if libcurlIncludeDir.startsWith "/" then "" else (pkg.srcDir.toString ++ "/")
...
extern_lib libleancurl pkg := do
  ...
  buildLeanSharedLib (pkg.nativeLibDir / name) #[leancurl] #[libcurlSharedLib]
